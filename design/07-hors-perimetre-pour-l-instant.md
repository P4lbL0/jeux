# §7 Hors périmètre pour l'instant

> [← Sommaire du design](../DESIGN.md)

---

Noté pour ne pas l'oublier, mais **on n'y touche pas** :

- Multijoueur (envisagé, écarté pour la v1)
- Simulation du village hors-ligne
- Serveur de leaderboard (le score, lui, est compté dès le début — §4.9)
- Sortie sur Steam / packaging en exécutable
- Son et musique
- **Les égouts** — noté le 11 août 2026, et **rangé ici par ta propre note** (« bien plus
  tard »). Ce que ça voudrait dire, pour qu'on sache ce qu'on reporte : un **deuxième
  niveau de carte** sous le village, donc une deuxième grille, un deuxième jeu de
  collisions, un passage entre les deux, une caméra qui change d'étage, et des monstres qui
  arrivent **par en dessous** — ce qui annulerait d'un coup la géographie du §4.6, les
  fronts, les enceintes du §4.20 et tout ce que le bloc 7 va bâtir. **Ce n'est pas un bloc,
  c'est un deuxième jeu**, et il ne peut pas se poser avant que le premier tienne debout.
