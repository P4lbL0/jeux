...
| `jeu/<date>-marche/` | La marche (§4.29) : on paraît seul, le dézoom d'entrée, quelqu'un qui vient, la question à la porte, le refus et le monde suivant, l'installation | `npx tsx .tmp/verifier-marche.ts` (script de contrôle, non commité) |
| `jeu/2026-09-21-lisibilite/` | Le texte avant et apres le passage a la densite reelle de l'ecran (§4.10, 21 septembre) : la scene entiere a 200 %, la ligne d'aide agrandie, le bandeau d'etat sur une fenetre 1600x900, et le dezoom maximal ou le vide vert disparait | `npx tsx .tmp/lisibilite.ts` (script de controle, non commite) |
| `jeu/2026-09-21-pause/` | Le menu de pause, le panneau des touches, les volumes et la ligne d'aide dépliée (§4.10, bloc 10) : ce que voit un joueur qui appuie sur Échap | `npx tsx .tmp/verifier-pause.ts` (script de contrôle, non commité) |
| `jeu/2026-09-21-memoire/` | La mémoire du village (§4.26, bloc 11) : les archives sous le tableau du village, et la fiche de quelqu'un qui a perdu un proche | `npx tsx .tmp/verifier-memoire.ts` (script de contrôle, non commité) |
| `jeu/2026-09-21-vie/` | La vie autonome (§4.27, bloc 12) : le village où chacun vaque, une rencontre, et les trois bulles au zoom | `npx tsx .tmp/verifier-vie.ts` (script de contrôle, non commité) |
